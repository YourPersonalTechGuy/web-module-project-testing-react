...
test('renders same number of options seasons are passed in', ()=>{
    ...
    expect(options).toHaveLength(testShow.show.seasons.length)
});

test('handleSelect is called when an season is selected', () => {
    const getFakeData = jest.fn();

    render(<Show show={testShow.show} selectedSeason={"none"} handleSelect={getFakeData}/>);

    const option = screen.queryByLabelText("Select A Season")

    userEvent.selectOptions(option, ["1"])
    expect(getFakeData).toHaveBeenCalledTimes(1);
});

test('component renders when no seasons are selected and when rerenders with a season passed in', () => {
    const { rerender } = render(<Show show={testShow.show} selectedSeason={"none"}/>)
    
    render(<Show show={testShow.show} selectedSeason={"none"}/>) 
    expect(screen.queryByTestId("episodes-container")).not.toBeInTheDocument();

    rerender(<Show show={testShow.show} selectedSeason={"0"}/>)
